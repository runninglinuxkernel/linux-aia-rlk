...
QEMU=qemu-system-riscv64-8 rootfs_arg="root=/dev/vda rootfstype=ext4 rw"
kernel_arg="earlycon=sbi console=ttyS0"
crash_arg=""
dyn_arg=""
...
run_qemu_debian(){
		...
		cmd="$QEMU -machine virt,aclint=on,aia=aplic-imsic -m 1024 $SMP \
			-nographic -kernel opensbi-riscv64-generic-fw_dynamic.bin -device loader,file=arch/riscv/boot/Image,addr=0x80200000 \
			-append \"$kernel_arg $debug_arg $rootfs_arg $crash_arg $dyn_arg\"\
			-drive if=none,file=$rootfs_image,id=hd0 -device virtio-blk-pci,drive=hd0\
			-device virtio-net-device,netdev=usernet -netdev user,id=usernet,hostfwd=tcp:127.0.0.1:5555-:22\
			--fsdev local,id=kmod_dev,path=./kmodules,security_model=none\
			-device virtio-9p-device,fsdev=kmod_dev,mount_tag=kmod_mount -device igb\
			$DBG"
		echo "running:"
		echo $cmd
		eval $cmd
